...
import pt.unl.adc.project.model.ChangeUserPasswordRequest;
...
/**
 * Esta classe é responsável por alterar a password de um utilizador.
 */
@Path("/changeuserpwd")
public class ChangeUserPasswordResource {
    ...
    public Response changeUserPassword(ChangeUserPasswordRequest request) {
        ...
        if (isBlank(request.getInput().getUsername())
                || isBlank(request.getInput().getOldPassword())
                || isBlank(request.getInput().getNewPassword())) {
            return invalidInputResponse("Missing username, oldPassword or newPassword");
        }
        ...
        if (!storedToken.getUsername().equals(request.getInput().getUsername())) {
            return Response.ok(
                    Map.of(
                            "status", FORBIDDEN,
        ...
        if (user == null || !user.getPassword().equals(request.getInput().getOldPassword())) {
            return Response.ok(
                    Map.of(
        ...
        user.setPassword(request.getInput().getNewPassword());
